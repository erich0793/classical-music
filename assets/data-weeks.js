...
window.WEEKS = [
{
  ...
  banner: "<b>先開好〈樂器圖鑑〉再開始。</b>本週的自我檢核要求你能單獨辨識八件樂器，任務 3 更點名三組最容易混淆的組合——側邊欄的<b>「器 樂器圖鑑」</b>有 14 件樂器的線稿與那三組的並列對照，<b>建議開在旁邊邊聽邊對</b>。",
  ...
  tasks: [
    ...
    "挑出三種你<b>最容易混淆</b>的樂器（常見組合：雙簧管 oboe vs. 單簧管 clarinet；小號 trumpet vs. 法國號 horn；中提琴 viola vs. 大提琴 cello），單獨搜尋該樂器的獨奏曲各聽 2 分鐘。<b>側邊欄的〈樂器圖鑑〉把這三組並列畫出來了</b>，先看圖再聽。"
  ],
  checks: [
...
},
{
  ...
  tasks: [
    ...
    "能辨識合奏段（ritornello）與獨奏段的交替",
    "能說出「獨奏 vs. 合奏」的對抗關係如何撐起整個樂章"
  ],
...
},
{
  ...
  checks: [
    "能說出古典交響曲四個樂章各自的速度與性格",
    "能辨識慢樂章的主題與變奏結構（回頭用第 3 週學的）"
  ],
},
{
...
},
{
  ...
  tasks: [
    ...
    "能說出 Beethoven 在規模與敘事上如何超出古典時期的慣例",
    "能察覺同一位作曲家早期與晚期在「可親近程度」上的落差"
  ],
...
},
{
  ...
  checks: [
    "能察覺 rubato——左右手不嚴格對齊所造成的彈性",
    "能說出「由敘事邏輯決定結構」與「由曲式範本決定結構」的差別"
  ],
},
{
...
},
{
  ...
  checks: [
    "能辨識同一個主題在不同樂章被變形後的樣貌",
    "能說出標題音樂與絕對音樂的差別，並各舉一例"
  ],
},
{
  ...
  checks: [
    "能察覺末樂章違反「快速、外向、強奏收尾」慣例時所造成的效果",
    "能辨識反覆出現的低音主題（passacaglia）"
  ],
},
{
...
},
{
  ...
  checks: [
    "能察覺不規則重音如何破壞節拍預期（跟著打拍子會失敗）",
    "能在二十世紀作品中辨識出仍然存在的古典形式"
  ],
},
{
  ...
  checks: [
    "能區分 Schoenberg 早期的調性作品與後期的無調性作品",
    "在無調性作品中，能改以音色、密度與寂靜作為聆聽標的"
  ],
},
{
  ...
  checks: [
    "能在極簡音樂中察覺「事情變了」與變化實際開始的落差",
    "能對同一部作品的兩種相反詮釋，各自說出支持的理由"
  ],
},
{
  ...
  checks: [
    "能用速度、力度對比、樂句處理、音色四個維度描述版本差異",
    "能說出自己偏好哪一種演出取向，並講得出為什麼"
  ],
},
{
...
},
{
  ...
  tasks: [
    ...
    "重聽區已有內容，且能說出其中的共同傾向（時期／作曲家／編制）",
    "已為接下來 6 個月選定一個具體的探索方向"
  ],
  gaps: {
    title: "本課程未涵蓋的主要區塊",
    items: [
