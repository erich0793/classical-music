(function () {
  ...
  var SCHEMES = {
    word: function (q, r, l) { return "https://www.kkbox.com/" + r + "/" + l + "/search?word=" + encodeURIComponent(q); },
    path: function (q, r, l) { return "https://www.kkbox.com/" + r + "/" + l + "/search/all/" + encodeURIComponent(q) + "/1"; },
    play: function (q) { return "https://play.kkbox.com/search/" + encodeURIComponent(q); }
  };
  function kk(q) { return SCHEMES[S.scheme in SCHEMES ? S.scheme : "word"](q, S.region, S.lang); }
  ...
  function versionHTML(v) {
    ...
    return '<li class="v ' + v.qa + '">' +
      ...
      "</div>" +
      '<div class="acts">' +
        '<a class="play" href="' + esc(kk(v.q)) + '" target="_blank" rel="noopener">在 KKBOX 開啟</a>' +
    ...
    if (wk.compare) {
      ...
      h += wk.compare.picks.map(function (p) {
        return '<li class="v ' + p.qa + '"><div><div class="p">' + esc(p.label) + "</div>" +
          ...
          '<div class="acts"><a class="play" href="' + esc(kk(p.q)) + '" target="_blank" rel="noopener">在 KKBOX 開啟</a>' +
  ...
  function homeHTML() {
    ...
    var h = '<div class="card"><h2>' + esc(CORE.meta.title) + "</h2>" +
      ...
      '<button class="iconbtn" data-open="settings">連結／排序設定</button></div></div>';
  ...
  function favHTML() {
    ...
    h += lines.map(function (L) {
      return '<li class="v ' + (L.top ? L.top.qa : "hifi") + '"><div>' +
        ...
        '<div class="acts"><a class="play" href="' + esc(kk(L.top ? L.top.q : L.w.q)) + '" target="_blank" rel="noopener">開啟</a>' +
  ...
  function settingsHTML() {
    ...
    return "<h3>連結與顯示設定</h3>" +
      ...
      '<div class="fld"><label>搜尋連結格式</label><select id="setScheme">' +
        opt("word", S.scheme, "www.kkbox.com/…/search?word=（預設）") +
        opt("path", S.scheme, "www.kkbox.com/…/search/all/…/1（舊格式）") +
        opt("play", S.scheme, "play.kkbox.com/search/…（網頁播放器）") + "</select>" +
        '<div class="hint">KKBOX 的搜尋網址格式可能隨網站改版變動。<b>若點擊連結後找不到結果，換一個格式試試。</b>另外，每個版本旁的「複製搜尋字串」按鈕永遠可用——貼進 KKBOX App 的搜尋列即可。</div></div>' +
      "<h3>版本排序</h3>" +
  ...
  document.addEventListener("change", function (e) {
    ...
    var map = { setRegion: "region", setLang: "lang", setScheme: "scheme", setTheme: "theme" };
